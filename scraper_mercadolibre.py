...
def _tokenize(value: str) -> set[str]:
    pattern = globals().get("WORD_RE") or re.compile(WORD_PATTERN, re.IGNORECASE)
    return {token.lower() for token in pattern.findall(_normalize(value))}
...
def _slugify_query(query: str) -> str:
    ...
    pattern = globals().get("WORD_RE") or re.compile(WORD_PATTERN, re.IGNORECASE)
    words = pattern.findall(normalized)
    return "-".join(words)
